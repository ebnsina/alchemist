...
<section class="mt-8">
	<button
		type="button"
		class="flex w-full items-center justify-between gap-4 rounded-xl border border-hairline px-4 py-3 text-left transition-colors hover:border-hairline-strong"
		onclick={() => (open = !open)}
		aria-expanded={open}
	>
		<span>
			<span class="text-sm font-semibold">Advanced</span>
			<span class="ml-2 text-xs text-muted">Chunks, pipeline steps, and the raw response</span>
		</span>
		<HugeiconsIcon
			icon={ArrowDown01Icon}
			size={17}
			strokeWidth={2}
			class="chevron flex-none text-muted {open ? 'rotate-180' : ''}"
		/>
	</button>
			...
			{:else}
				<div>
					...
					{#if byRendition.length === 0}
						<p class="mt-3 text-sm text-muted">No chunks yet.</p>
					{:else}
							...
							{#each byRendition as group (group.name)}
								...
								<div class="card p-4">
									<div class="flex items-baseline justify-between text-xs">
										...
										<span class="text-muted tabular-nums">{done} of {group.items.length}</span>
									</div>
									<div class="mt-2.5 flex flex-wrap gap-1">
										{#each group.items as c (c.index)}
											<span
												class="h-3.5 w-3.5 rounded-[3px] {c.done ? 'bg-brand-mid' : 'bg-white/10'}"
									...
									{#each activity as a, i (a.step + a.queued_at + i)}
										<tr class="border-t border-hairline">
											...
											<td class="px-4 py-2.5 text-right tabular-nums">
												{a.attempt}{#if a.failures > 0}<span class="text-[#fca5a5]"> · {a.failures} failed</span>{/if}
											</td>
											<td class="px-4 py-2.5 text-right text-muted tabular-nums">{clock(a.started_at)}</td>
				...
				<div>
					...
					<p class="mt-1 text-xs text-muted">The same call your own code would make.</p>
					<div class="mt-3">
