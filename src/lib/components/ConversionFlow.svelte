...
<figure class="card shine mx-auto mt-14 max-w-2xl p-4 text-left sm:p-6">
	...
	<ol class="mb-5 flex items-center gap-2 text-xs">
		{#each steps as s, i (s.label)}
			...
			>
				...
				{s.label}
				{#if i < steps.length - 1}
					<span class="ml-1 h-px w-5 bg-white/10 sm:w-8" aria-hidden="true"></span>
				{/if}
			</li>
		{/each}
	</ol>
			...
			>
				{#if stage === 0}
					...
					>
						...
						>
							<svg class="h-5 w-5 flex-none text-emerald-light" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4.5 6.5h11a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2h-11a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2zM17.5 10.5l4-2.5v8l-4-2.5z" /></svg>
				...
				{:else if stage === 1}
					<div class="flex h-full flex-col justify-center gap-4">
						...
						<div class="h-1.5 overflow-hidden rounded-full bg-white/10">
							<div
								class="h-full rounded-full bg-emerald transition-[width] duration-100 ease-linear"
						...
						<ul class="flex flex-wrap gap-2">
							{#each outputs as o (o.name)}
								{#if elapsed >= o.at}
									...
									>
										<svg class="h-3 w-3 flex-none text-emerald-light" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m5 12.5 4.5 4.5L19 7" /></svg>
										{o.name}
									</li>
								{/if}
							{/each}
				...
				{:else}
					<div class="flex h-full flex-col justify-center gap-4">
						<div class="flex items-center gap-3">
							<span
								class="flex h-9 w-9 flex-none items-center justify-center rounded-full bg-emerald text-body"
						...
						>
							<svg class="h-4 w-4 flex-none text-emerald-light" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9.5 14.5a4 4 0 0 0 5.7 0l3-3a4 4 0 0 0-5.7-5.7l-1 1M14.5 9.5a4 4 0 0 0-5.7 0l-3 3a4 4 0 0 0 5.7 5.7l1-1" /></svg>
							<span class="truncate font-mono text-xs text-muted">alchemist.video/w/8fc21a</span>
							<span class="ml-auto flex-none text-xs text-emerald-light">Copy</span>
						</div>
					</div>
				{/if}
			</div>
