<script lang="ts">
	...
	const busy = $derived(
		assets.some((a) => !['ready', 'failed', 'partially_ready'].includes(a.state)) ||
			streams.some((s) => s.state === 'armed' || s.state === 'live')
	);
	...
	// Live is a plan, not a given, and asking for streams without it is a 403 that
	// says nothing to anybody reading this page.
	async function loadLive() {
		try {
			liveOn = (await whoami()).live_enabled;
			if (liveOn) streams = (await listLiveStreams()).live_streams;
		} catch {
			liveOn = false;
		}
	}
	...
	const onAir = $derived(streams.filter((s) => s.state === 'live'));
	const waiting = $derived(streams.filter((s) => s.state === 'armed'));
...
<Seo title="Overview — Alchemist" description="Your account at a glance." />
...
{#if onAir.length || waiting.length}
	<!-- Above everything else: a broadcast going out is the only thing here that
	     cannot wait until the page has been read. -->
	<section class="card mt-7">
		<div class="flex flex-wrap items-baseline justify-between gap-3">
			<p class="title flex items-center gap-2">
				<HugeiconsIcon icon={LiveStreaming01Icon} size={16} strokeWidth={1.8} />
				{onAir.length ? `${onAir.length} on air now` : `${waiting.length} waiting for an encoder`}
			</p>
			<a href="/app/live/" class="link text-sm">Live streams</a>
		</div>
		<ul class="mt-4 grid gap-2">
			{#each [...onAir, ...waiting] as s (s.id)}
				<li class="flex items-center justify-between gap-3">
					<a href="/app/live/{s.id}/" class="min-w-0 truncate text-sm font-medium hover:opacity-80">
						{s.name}
					</a>
					<span class="chip flex-none {s.state === 'live' ? 'chip-on' : ''}">
						{s.state === 'live' ? 'On air' : 'Waiting'}
					</span>
				</li>
			{/each}
		</ul>
...
<div class="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
	...
	<section class="card min-w-0">
		<div class="flex flex-wrap items-baseline justify-between gap-3">
			...
			<a href="/app/videos/" class="link flex-none text-sm whitespace-nowrap">Open them</a>
		</div>
